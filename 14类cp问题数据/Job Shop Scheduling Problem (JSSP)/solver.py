...
def parse_jssp_json(file_path):
    ...
    for job in data["jobs"]:
        ...
        for op in job["operations"]:
            operations.append((op["machine"], op["duration"]))
        jobs_data.append(operations)
    
    return num_jobs, num_machines, jobs_data, data.get("id", os.path.basename(file_path))

def solve_jssp(file_path, time_limit=60):
    print(f"正在求解: {file_path}")
    num_jobs, num_machines, jobs_data, instance_id = parse_jssp_json(file_path)
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id, (machine, duration) in enumerate(job):
            ...
            task_intervals[(job_id, task_id)] = interval_var
